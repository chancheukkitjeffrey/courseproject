...
int choice; // Option selected for main menu
bool same; // Bool to determine if a randomized ID is taken. True ==> yes is same, False ==> no not same
long tempID; // Storage of ID generated by the rand function. Only assigned as newEmp ID once checked against all employee IDs in the employees vector

vector <employee> employees; // Vector that contains all objects of the employee class. ***HAVE TO FIND A WAY TO SAVE TO A FILE ONCE DONE RUNNING THE PROGRAM***
...
int main () {
  cout << endl << "~~~Staff Management System~~~" << endl <<
          "1: Insert new employee information" << endl << 
  ...
  if (choice > 5 || choice < 1) {
    cout << "Error: Input value was not between 1 and 5. Restarting..." << endl << endl;
    main ();
  }
  
  else if (choice == 1) {
    cout << "You have selected option 1. How many new employees are you entering? Please type in the total (just 1 if 1, 2 if 2 etc.) and press enter." << endl;
